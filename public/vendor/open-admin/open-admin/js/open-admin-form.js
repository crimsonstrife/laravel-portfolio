...
admin.form = {
    ...
    beforeSave: function () {
        for (let i in this.beforeSaveCallbacks) {
    ...
    addAjaxSubmit: function () {
        ...
        Array.from(document.getElementsByTagName("form")).forEach((form) => {
            if (
                form.getAttribute("pjax-container") != null &&
                !form.classList.contains("has-ajax-handler")
            ) {
    ...
    footer: function () {
        document.querySelectorAll(".after-submit").forEach((check) => {
            check.addEventListener("click", function () {
                document
                    .querySelectorAll(
                        ".after-submit:not([value='" + this.value + "']"
                    )
                    .forEach((other) => {
                        other.checked = false;
                    });
    ...
    check_tab_errors() {
        let errors = document.querySelectorAll(
            ".tab-pane .has-error, .was-validated .tab-pane .form-control:invalid"
        );
        if (this.tabs_ref.length && errors) {
            ...
            errors.forEach((error) => {
                ...
                document
                    .querySelector('li a[href="' + tabId + '"] i')
            ...
            if (first_tab) {
                let errorTab = document.querySelector(
                    '.nav-tabs a[href="' + first_tab + '"]'
                );
    ...
    disable_cascaded_forms: function (selector) {
        document
            ...
            .addEventListener("submit", function (event) {
                let elems = event.target.querySelectorAll(
                    "div.cascade-group.d-none input"
                );
                if (elems) {
                    elems.forEach((field) => {
